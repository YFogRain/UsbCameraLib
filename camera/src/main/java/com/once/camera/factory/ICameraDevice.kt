...
abstract class ICameraDevice {
	 ...
	 * 打开预览
	 */
	abstract suspend fun startPreview(): Boolean
	
	/**
	 * 关闭预览
	 */
	abstract fun stopPreview(): Boolean
	 ...
	 */
	abstract suspend fun takePicture(cropWidth:Int = -1,cropHeight: Int = -1): Bitmap?
	
	/**
	 * 获取当前的设备预览方向
	 */
	protected fun loadOrientation(sensorOrientation: Int, isFont: Boolean, rotation: Int): Int {
		Log.d("cameraDevice", "当前的页面方向为 = $rotation")
		...
		Log.d("cameraDevice", "根据跳转方向为 = $degrees")
		Log.d("cameraDevice", "传感器方向为 = $sensorOrientation")
		return if (isFont) {

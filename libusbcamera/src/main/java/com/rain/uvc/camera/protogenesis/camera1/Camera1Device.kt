...
package com.rain.uvc.camera.protogenesis.camera1
...
class Camera1Device(camera: Camera, cameraId: Int, acRotation: Int) : ICameraDevice() {
	 ...
	 * 拍照的回调
	 */
	private val pictureContinuation = AtomicReference<((ByteArray, Int, Int) -> Unit)?>(null)
	...
	override fun startPreview() {
		...
		camera.setPreviewCallback { data, _ ->
			...
			pictureContinuation.getAndSet(null)?.let {
				it.invoke(data, previewSize.width, previewSize.height)
	...
	override fun <V> getParameter(key: Parameters.Key<V>): V? {
		...
		return when (key.type) {
			...
			CameraParameterType.PREVIEW_ORIENTATION -> this.mDisplayOrientation
	...
	override fun <T> getSupportParameters(supportKey: SupportParameters.Key<T>): T? {
		...
		return when (supportKey.type) {
			...
			CameraParameterType.PREVIEW_SIZE -> getSupportPreviewSizes(parameters)
	...
	override suspend fun takePicture(cropWidth: Int, cropHeight: Int): Bitmap? {
		return withTimeoutOrNull(3000) {
			suspendCancellableCoroutine { continuation ->
				// 如果已经有拍照请求，直接失败（避免并发）
				val callback: (ByteArray, Int, Int) -> Unit = { bytes, width, height ->
					val bmp = runCatching {
						bytes.toBitmap(ImageFormat.NV21, width, height)?.cropBitmap(
...
//		//白平衡
	 ...
	 * 获取支持的分辨率列表
	 */
	private fun getSupportPreviewSizes(parameters: Camera.Parameters): Array<CameraSupportSize>? {
		val supportedPreviewFormats = parameters.supportedPreviewFormats?.filter { it.isUserFormat() }
		val supportedPreviewSizes = parameters.supportedPreviewSizes
		// 没有数据，直接return
		if (supportedPreviewFormats.isNullOrEmpty() || supportedPreviewSizes.isNullOrEmpty()) return null
		val previewSizeList = supportedPreviewSizes.map { CameraSize(it.width, it.height) }
		return Array(supportedPreviewFormats.size) {
			CameraSupportSize(supportedPreviewFormats[it].toPreviewFormat(), previewSizeList)
		}
	}
}
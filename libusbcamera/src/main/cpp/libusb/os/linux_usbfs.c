...
static int op_init(struct libusb_context *ctx, const char *usbFsPath) {
    ...
    if (!kernel_version_ge(&kversion, 2, 6, 32)) {
        LOG_E("当前设备的linux内核版本低于2.6.32，本地版本为:%d.%d.%d", kversion.major,
              kversion.minor,
...
static int parse_config_descriptors(struct libusb_device *dev) {
    ...
    for (idx = 0; idx < num_configs; idx++) {
        ...
        if (remaining < LIBUSB_DT_CONFIG_SIZE) {
            LOG_E("剩余数据的长度不足一个配置描述符的基本长度,%zu/%d", remaining,
        ...
        if (config_desc->bDescriptorType != LIBUSB_DT_CONFIG) {
            ...
            LOG_E("bLength长度小于标准配置描述符长度,bLength:%u", config_desc->bLength);
            return LIBUSB_ERROR_IO;
        }
        //读取配置描述符的总长度
        ...
        if (config_len < LIBUSB_DT_CONFIG_SIZE) {
            LOG_E("总长度小于标准配置描述符的长度,config_len:%u", config_len);
            return LIBUSB_ERROR_IO;
        }
        //Sysfs 描述符处理
        if (priv->sysfs_dir) {
...
static int usbfs_get_active_config(struct libusb_device *dev, int fd) {
    ...
    struct usbfs_ctrltransfer ctrl = {
            .bmRequestType = LIBUSB_ENDPOINT_IN, //请求类型，LIBUSB_ENDPOINT_IN 表示从设备读取数据
    ...
    LOG_E("读取到的控制配置支持结果：%d", r);
    //调用失败。
    if (r < 0) {
        //如果设备已断开，返回错误码 LIBUSB_ERROR_NO_DEVICE
        if (errno == ENODEV)
            return LIBUSB_ERROR_NO_DEVICE;
        LOG_E("获取配置信息失败：%d", errno);
        if (priv->config_descriptors)
...
static int initialize_device(struct libusb_device *dev, uint8_t busnum,
    ...
    //判断文件描述符是否无效
    if (fd < 0) {
        LOG_E("无法确定设备的活动配置描述符，因为缺少对 usbfs 的读写访问");
        //推测活动配置
        if (priv->config_descriptors)
...
static int initialize_handle(struct libusb_device_handle *handle, int fd) {
    ...
    if (r < 0) {
        if (errno == ENOTTY)
            LOG_D("获取设备能力未活跃, errno=%d", errno);
        else
            LOG_E("获取设备能力失败, errno=%d", errno);
        hpriv->caps = USBFS_CAP_BULK_CONTINUATION;
    }
    LOG_D("获取设备能力结果：%d", hpriv->caps);
...
static int handle_bulk_completion(struct usbi_transfer *itransfer,
    ...
    if (tpriv->reap_action != NORMAL) {
        ...
        if (urb->actual_length > 0) {
            ...
            usbi_dbg(TRANSFER_CTX(transfer), "received %d bytes of surplus data",
                     urb->actual_length);
...
static int handle_iso_completion(struct usbi_transfer *itransfer,
    ...
    for (i = 0; i < urb->number_of_packets; i++) {
        ...
        switch (urb_desc->status) {
            ...
            case -ENOSR:
            case -EXDEV:
                usbi_dbg(TRANSFER_CTX(transfer), "packet %d - low-level USB error %d", i,
                         urb_desc->status);
...
static int reap_for_handle(struct libusb_device_handle *handle) {
    ...
    usbi_dbg(HANDLE_CTX(handle), "urb type=%u status=%d transferred=%d", urb->type, urb->status,
             urb->actual_length);
...
android_generate_device(struct libusb_context *ctx, struct libusb_device **dev, int fd, int busNum,
    ...
    LOG_D("当前设备的设备总线和fd:%d/%d::%d", busNum, devAddress, fd);
    //分配设备对象
    *dev = usbi_alloc_device(ctx, session_id);
    if (!dev)
        return LIBUSB_ERROR_NO_MEM;

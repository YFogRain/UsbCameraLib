...
static int parse_endpoint(struct libusb_context *ctx,
                          struct libusb_endpoint_descriptor *endpoint, const uint8_t *buffer,
                          int size) {
...
static int parse_interface(libusb_context *ctx,
                           struct libusb_interface *usb_interface, const uint8_t *buffer,
                           int size) {
    int len;
    int r;
    int parsed = 0;
...
static int parse_configuration(struct libusb_context *ctx,
                               struct libusb_config_descriptor *config, const uint8_t *buffer,
                               int size) {
    uint8_t i;
    int r;
    const struct usbi_descriptor_header *header;
...
static int get_config_descriptor(struct libusb_device *dev, uint8_t config_idx,
    ...
    if (r < 0)
        return r;
    if (r < LIBUSB_DT_CONFIG_SIZE) {
        LOG_E("short config descriptor read %d/%d", r, LIBUSB_DT_CONFIG_SIZE);
        return LIBUSB_ERROR_IO;
    } else if (r != (int) size) {
        LOG_D("short config descriptor read %d/%d", r, (int) size);
    }
...
int API_EXPORTED libusb_get_config_descriptor(libusb_device *dev,
    ...
    uint8_t *buf;
    int r;
    LOG_D("当前的index:%u", config_index);
    if (config_index >= dev->device_descriptor.bNumConfigurations)
        return LIBUSB_ERROR_NOT_FOUND;
    LOG_D("开始获取对应的描述符配置:%u", dev->device_descriptor.bNumConfigurations);

...
private:
    ...
    uvc_frame_format getPreviewFormat() {
        ...
        case PREVIEW_FORMAT_MJPEG:
            LOG_D("当前使用 PREVIEW_FORMAT_MJPEG");
            return UVC_FRAME_FORMAT_MJPEG;
        case PREVIEW_FORMAT_YUY2:
            LOG_D("当前使用 UVC_FRAME_FORMAT_YUYV");
        ...
        case PREVIEW_FORMAT_BGR:
            LOG_D("当前使用 UVC_FRAME_FORMAT_BGR");
    ...
    int uvc_format_to_mode(uvc_frame_format format) {
...
public:
    CameraStreamUsbImpl(uvc_device_handle_t *mDeviceHandle);

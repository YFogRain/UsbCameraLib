...
extern "C" JNIEXPORT jboolean JNICALL
Java_com_rain_uvc_utils_CameraNativeUtils_nativeStartPreview(JNIEnv *env, jclass clazz, jlong nativeId) {
...
extern "C" JNIEXPORT jboolean JNICALL
Java_com_rain_uvc_utils_CameraNativeUtils_nativeStopPreview(JNIEnv *env, jclass clazz, jlong nativeId) {
    auto *camera = reinterpret_cast<ICameraFactory *>(nativeId);
    if (camera) {
...
extern "C" JNIEXPORT jboolean JNICALL
Java_com_rain_uvc_utils_CameraNativeUtils_nativeSetDisplaySurface(JNIEnv *env, jclass clazz, jlong nativeId, jobject jSurface) {
    auto *camera = reinterpret_cast<ICameraFactory *>(nativeId);
    if (camera) {
...
extern "C" JNIEXPORT jboolean JNICALL
Java_com_rain_uvc_utils_CameraNativeUtils_nativeSetPreviewSize(JNIEnv *env, jclass clazz, jlong nativeId, jint width, jint height, jint format) {
    auto *camera = reinterpret_cast<ICameraFactory *>(nativeId);
    if (camera) {
...
extern "C" JNIEXPORT jboolean JNICALL
Java_com_rain_uvc_utils_CameraNativeUtils_setPreviewListener(JNIEnv *env, jclass clazz, jlong nativeId, jobject listener, jint mode) {
    auto *camera = reinterpret_cast<ICameraFactory *>(nativeId);
    if (camera) {
        JavaVM *vm;
        env->GetJavaVM(&vm);
...
Java_com_rain_uvc_utils_CameraNativeUtils_nativeLoadV4L2Devices(JNIEnv *env, jclass clazz) {
    ...
    //创建指定大小的集合
    jobjectArray deviceArray = env->NewObjectArray(vector.size(), env->FindClass("java/lang/String"), nullptr);

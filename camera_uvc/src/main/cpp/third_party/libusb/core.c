...
struct libusb_device *usbi_alloc_device(struct libusb_context *ctx,
    ...
    if (!libusb_has_capability(LIBUSB_CAP_HAS_HOTPLUG)) {
        ...
        LOG_D("libusb-core","当前不支持热插拔，添加设备到全局活动列表");
...
int usbi_sanitize_device(struct libusb_device *dev) {
    ...
    if (dev->device_descriptor.bLength != LIBUSB_DT_DEVICE_SIZE ||
        ...
        LOG_E("libusb-core","初始化设备描述符失败：bLength：%d", dev->device_descriptor.bLength);
        return LIBUSB_ERROR_IO;
    }

    num_configurations = dev->device_descriptor.bNumConfigurations;
    if (num_configurations > USB_MAXCONFIG) {
        LOG_E("libusb-core","配置总数超过了最大值：%d", num_configurations);
        return LIBUSB_ERROR_IO;
    } else if (0 == num_configurations) {
        LOG_E("libusb-core","zero configurations, maybe an unauthorized device：%d", num_configurations);
    }
    return 0;
}
...
int API_EXPORTED libusb_open(libusb_device *dev, libusb_device_handle **dev_handle, int fd) {
    ...
    size_t priv_size = usbi_backend.device_handle_priv_size;
    int r;
    LOG_D("libusb-core","打开设备。 %d.%d", dev->bus_number, dev->device_address);
    // 检查设备是否已附加
    ...
    if (r < 0) {//打开失败
        LOG_E("libusb-core","open %d.%d returns %d", dev->bus_number, dev->device_address, r);
        libusb_unref_device(dev);
        usbi_mutex_destroy(&_dev_handle->lock);
        free(_dev_handle);
        return r;
            ...
            case LIBUSB_OPTION_LOG_LEVEL:
                ...
                if (!ctx->debug_fixed) {
                    ...
                    if (is_default_context)
                        usbi_atomic_store(&default_debug_level, CLAMP(arg, LIBUSB_LOG_LEVEL_NONE,
...
static void log_v(struct libusb_context *ctx, enum libusb_log_level level,
    ...
    if (ctx) {
        ...
        ctx_level = default_level_value < 0 ? get_env_debug_level()
                                            : (enum libusb_log_level) default_level_value;
    }
    ...
    if (global_debug) {
        ...
        if (!has_debug_header_been_displayed) {
            ...
            log_str(LIBUSB_LOG_LEVEL_DEBUG,
                    "[timestamp] [threadID] facility level [function call] <message>" USBI_LOG_LINE_END);
            log_str(LIBUSB_LOG_LEVEL_DEBUG,
                    "--------------------------------------------------------------------------------" USBI_LOG_LINE_END);
        }
        ...
        header_len = snprintf(buf, sizeof(buf),
                              ...
                              (long) timestamp.tv_sec, (long) (timestamp.tv_nsec / 1000L),
...
LIBUSB_CALL libusb_get_device_with_fd(libusb_context *ctx, int fd, int busNum, int devAddress) {
    ...
    if (ret) {
        LOG_E("libusb-core","获取设备失败了:err=%d", ret);
        device = NULL;
    }
    return device;
}
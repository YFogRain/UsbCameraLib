...
class VideoEngine {
	 ...
	 * 初始化surface
	 */
	fun prepareSurface(width: Int, height: Int, fps: Int = mFps): Boolean {
	...
	/**
	 * 设置录制帧率（需要在 prepareSurface 或 start 前调用）
	 */
	fun setFps(fps: Int) {
		mFps = fps
		Log.d("VideoEngine", "设置帧率: $fps")
	}
	
	private fun prepareCodec(surface: Surface, width: Int, height: Int, fps: Int = 30): Boolean {
	 ...
	 * 开始录制
	 */
	fun start(width: Int, height: Int, fps: Int = mFps): Boolean {
